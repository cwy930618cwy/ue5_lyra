...
**不建子文件夹**，所有 md 文件平铺在知识点总目录下，用编号前缀排序。代码统一放一个 `代码/` 文件夹，整个知识点共享同一份源码，不每个子章节复制。

**示例目录结构：**
```
1.1.1_Experience系统总览/
├── README.md                    ← 总入口（学习路线图 + 核心类速查）
├── 01_为什么需要Experience系统.md
├── 02_核心文件清单.md
├── 03_完整工作流程时间线.md
├── 04_加载状态机.md
├── 05_依赖关系图.md
...
1. **只存一份**：整个知识点目录共享一个 `代码/` 文件夹，不每个子章节各复制一份
2. **只搬相关的**：把与该知识点直接相关的源码文件（`.h` / `.cpp`）复制进来，不是无脑全搬
3. **正文指引去看**：md 里告诉读者去 `代码/` 文件夹看完整源码，不在末尾贴完整代码
4. **关键片段可以贴**：正文中可以贴几行关键代码说明核心逻辑，但要同时指引"完整实现见 `代码/xxx.h`"
5. **代码文件必须加中文注释**：`代码/` 里的 `.h` / `.cpp` 文件，每个方法、关键字段、关键逻辑都要加中文注释，让读者能直接看懂。不是改代码逻辑，是加注释降低阅读门槛

### 写作要点

1. **开头问题驱动**：用一个具体场景引入，让读者立刻知道"为什么要学这个"
2. **对比强化理解**：传统做法 vs Lyra 做法，突出改进点
3. **代码即文档**：正文只贴关键片段（几行），完整源码放 `代码/` 文件夹让读者自己看，不要在末尾贴大段完整代码
4. **行号可追溯**：方便读者打开源码对照验证
5. **流程图收尾**：把所有知识点串成一条线，形成完整认知
6. **不建子文件夹**：md 文件平铺在总目录下，用编号前缀排序（如 `01_xxx.md`、`02_xxx.md`）
